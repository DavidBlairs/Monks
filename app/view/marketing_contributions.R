box::use(
  shiny[tags, NS, moduleServer, uiOutput, renderUI, outputOptions, renderPlot, actionButton, reactive, observe, observeEvent, updateDateRangeInput, updateSelectInput, req, plotOutput, selectInput, dateRangeInput, tagList],
  dplyr[tbl, rename, bind_rows, collect, pull, distinct, select],
  shinyWidgets[dropMenu]
)

box::use(
  app/logic/plot_marketing_contributions[plot_marketing_contributions]
)
...
server <- function(id, con, generated_filters) {
  moduleServer(id, function(input, output, session) {
    ...
    output$display_view_plot <- renderPlot({
        ...
        plot_marketing_contributions(con, selected_options())
    })
